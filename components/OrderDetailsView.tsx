...
export default function OrderDetailsView({ details, confirmation = false, footer }: { details: OrderDetails; confirmation?: boolean; footer?: React.ReactNode }) {
  ...
  return <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    <Section title="Delivery address"><Text style={styles.strong}>{order.addressLabel}</Text><Text style={styles.muted}>{address}</Text>{order.deliveryLandmarkDirections ? <Text style={styles.muted}>{order.deliveryLandmarkDirections}</Text> : null}{order.deliveryContactPhone ? <Text style={styles.muted}>{order.deliveryContactPhone}</Text> : null}</Section>
